...
import com.project.basicboard.util.FormDataEncoder;
...
class ArticleControllerTest {
    ...
    // ArticleControllerTest가 ArticleService까지 들어가지않고, 단순히 입출력만을 위한 테스트가 되기 위해서
    // ArticleService와의 연결을 끊어줘야하는데  이때 @MockBean을 씀
    ...
    @WithMockUser //유저정보를 목킹해서 넣어줌(실제 구현되어 있는 인증정보를 사용하는게 아니라 인증되었다고 가정하는 것)
...
*/
    ...
    // @WithMockUser와 달리 구현한 인증정보를 사용하여 인증받는 것(유저정보도 실제로 DB에 있어야 함)
    //TestExecutionEvent.TEST_EXECUTION  : 테스트 실행하기 직전에 이 셋업을 진행해라.
    //unoTest 계정은 TestSecurityConfig 에서 만들어 놓음
